...
Benchmark.prototype.run = function () {
  ...
  this.on('done', function (id) {
    ...
    var avgRT = use / that.requests;
    var qps = that.requests / use * 1000;
    console.log(rtCounts)
    var total = that._finished;
    var rates = {};
    for (var t in rtCounts) {
      rates[t] = rtCounts[t] / total * 100;
    }

    console.log(REPORT, total, that.concurrency, use / 1000,
